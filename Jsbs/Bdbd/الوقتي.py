...
from . import edit_delete, jmub, logging
...
autopic_path = os.path.join(os.getcwd(), "jmub", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "jmub", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "jmub", "photo_pfp.png")

digitalpfp = (
    gvarstatus("DIGITAL_PIC") or "https://graph.org/file/cc6c91f662920d54e85de.jpg"
)
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await jmub.upload_file(autophoto_path)
        try:
            if i > 0:
                await jmub(
                    functions.photos.DeletePhotosRequest(
                        await jmub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await jmub(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await jmub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await jmub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@jmub.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
jmub.loop.create_task(digitalpicloop())
jmub.loop.create_task(autoname_loop())
jmub.loop.create_task(autobio_loop())
