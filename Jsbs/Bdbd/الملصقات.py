...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"ꫀ𝑑𝑤𝑎𝑟𝑑 𝑘ꫀꪀ𝑤𝑎𝑦_{userid}_{pack}_anim"
    if is_video:
...
@jmub.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@jmub.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@jmub.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@jmub.ar_cmd(pattern="معلومات_ألملصق$")
